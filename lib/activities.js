...
Meteor.methods({
  createActivity: function(activity, postType, loc) {
    ...
    check(postType, Object);
    check(loc, Match.OneOf(Object, null));
    
    activity.userId = Meteor.userId();
    activity.userAvatar = (Meteor.user().services.twitter) ? Meteor.user().services.twitter.profile_image_url_https : Meteor.user().profile.picture;
    ...
    if (! this.isSimulation && postType.tweet){
      tweetActivity(activity)
    }else if(! this.isSimulation && postType.fbPost){
...
if (Meteor.isServer) {
  ...
  var facebookActivity = function(activity){
    // we need to strip the "data:image/jpeg;base64," bit off the data url
    var image = activity.image.replace(/^data.*base64,/, '');
    var url = 'https://graph.facebook.com'+
      Meteor.user().profile.facebook.id+
      "/photos?access_token="+
      Meteor.user().profile.facebook.accessToken;
    ...
    if (response.statusCode !== 200)
      throw new Meteor.Error(500, 'Unable to post image to facebook');
...
Meteor.startup(function() {
  if (Meteor.isServer && Activities.find().count() === 0) {
    Activities.insert({
      recipeName: '30-off-nineWest',
      text: 'I only spend this much on this deal - incredible!',
      image: '/img/activity/activity-placeholder-strawberry-640x640.jpg',
      userAvatar: '/img/app/avatar-1.jpg',
      userName: 'Jane Doe',
      place: 'Perimeter Mall, Atlanta',
      date: new Date
    });
  }
});
